...
<%
	...
	Users u_edit = (Users) request.getAttribute("user_edit");
%>
		...
		<form name="user" method="post" action="accedi">
			<TABLE class="tcontent" align="center" cellspacing="0" style="width:40% ">
				<TR>
					<TD><B>Username : </B></TD>
					<TD>
						<input type="text" name="u" value="<%= u_edit.getUsername() %>" disabled>
					</TD>
				</TR>
				<TR>
					<TD><B>Password : </B></TD>
					<TD>
						<input type="text" name="password" value="<%= u_edit.getPassword() %>">
					</TD>
				</TR>
				<TR>
					<TD><B>Tipo : </B></TD>
					<TD>
						<select name="tipo">
							<option value="1" <% if(u_edit.getTipo().intValue() == 1) out.print(" selected "); %>>1 - Gestione Soci</option>
							<option value="2" <% if(u_edit.getTipo().intValue() == 2) out.print(" selected "); %>>2 - Gestione Bilancio + Soci</option>
							<option value="3" <% if(u_edit.getTipo().intValue() == 3) out.print(" selected "); %>>3 - GESTORE</option>
						</select>
					</TD>
				</TR>
			...
			<input type="hidden" name="username" value="<%= u_edit.getUsername() %>">
